...
public class ArmyManagerGreen : ArmyManager
{
    public override void ArmyElementHasBeenKilled(GameObject go)
	{
		...
		if (m_ArmyElements.Count == 0)
		{
			...
			GUIUtility.systemCopyBuffer =  "Green\t" + ((int)Timer.Value).ToString() + "\t"+nDrones.ToString()+"\t"+nTurrets.ToString()+"\t"+health.ToString();
